...
class MediaController extends Controller
{
    ...
    public function catalogue($tipologia){
        $data = [];
        if ($tipologia === 'film') {
            $data = $this->database['films'];
        } elseif ($tipologia === 'serie') {
            $data = $this->database['series'];
        }
        return view('catalogue', compact('data', 'tipologia'));
    }
    public function detail($uri){
        $media = null;
        
        foreach ($this->database['films'] as $film) {
            if ($film['uri'] === $uri) {
                $media = $film;
                break;
            }
        ...
        if (!$media) {
            foreach ($this->database['series'] as $serie) {
                if ($serie['uri'] === $uri) {
                    $media = $serie;
                    break;
                }
        ...
        if ($media) {
            return view('detail', ['media' => $media]);
        } else {
            abort(404);
        }
    }
